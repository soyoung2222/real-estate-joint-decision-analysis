...
> **Acceptance Criteria 확장본**  
> 원본 `prd_v0.1_20260820.md`의 본문과 FR-01~FR-25 요구사항은 변경하지 않고, 구현 최소 완료 조건만 추가했다.
...
- **AC-01-01:** A가 `같이 고르기`에 진입해 관심매물 1~5개를 선택하면, 시스템은 선택한 매물 ID만 포함한 공유 객체 초안을 구성한다.
- **AC-01-02:** A가 6개째 매물을 추가하려 하면 시스템은 해당 매물을 초안에 추가하지 않고 최대 5개 제한을 표시한다.
...
- **AC-02-01:** A는 다른 기본 조건보다 먼저 예산을 입력해야 하며, 유효한 예산이 없으면 기본 입력을 완료할 수 없다.
- **AC-02-02:** A가 예산을 입력하면 시스템은 출퇴근 여부를 질문하고, `출근함`을 선택한 경우에만 출근지와 이동수단 입력을 요청한다.
- **AC-02-03:** A가 `출근 안 함`을 선택하면 출근지·이동수단을 요구하지 않고 통근시간과 교통비를 판정 대상에서 제외한다.
...
- **AC-03-01:** A 또는 B가 첫 결과 후 `더 좁히기`를 선택하면 추가 필수 조건을 한 번에 하나씩 추가할 수 있고, 추가하지 않고 종료하는 것도 가능하다.
- **AC-03-02:** 추가한 조건은 해당 사용자에게 귀속되어 저장되고 즉시 현재 후보에 재적용되며, 사용자당 4개를 초과해 추가할 수 없다.
...
- **AC-04-01:** 사용자가 선호를 입력하면 시스템은 자유 문장 0~3개를 해당 사람의 카드에 저장하고 매물별 충족·미충족 판정에는 사용하지 않는다.
- **AC-04-02:** 사용자가 `확인 필요` 항목을 입력하면 시스템은 이를 방문 전 중개사 질문 목록으로 저장하며, 방문 후 공통 체크리스트와 동일한 항목이나 상태로 처리하지 않는다.
...
- **AC-05-01:** A가 관계 유형을 선택하고 공유를 실행하면 시스템은 초대 링크와 보조 코드를 발급하고 B 참여 전까지 대기 상태를 표시한다.
- **AC-05-02:** 관계 유형은 A의 초대 생성 시 한 번 저장하며, B의 참여 과정에서 같은 관계 유형을 다시 입력하도록 요구하지 않는다.
...
- **AC-06-01:** B가 유효한 링크 또는 코드로 참여하면, 시스템은 B의 조건 입력 화면보다 먼저 공유된 후보 최대 5개와 A의 선호 카드를 표시한다.
...
- **AC-07-01:** 비로그인 B가 예산과 조건을 입력하면 시스템은 입력값을 해당 초대 코드에 연결해 임시 저장하고 다른 초대의 조건과 섞지 않는다.
- **AC-07-02:** B가 로그인하면 해당 초대 코드에 임시 저장된 조건을 B 계정으로 이관하고 이후 동일 조건을 계정 기준으로 사용할 수 있다.
- **AC-07-03:** 로그인으로 이관되지 않은 임시 조건은 마지막 접근일로부터 30일이 지나면 삭제한다.
...
- **AC-08-01:** 비로그인 열람이 허용되는 경우, B가 첫 결과를 보기 전에는 결과 저장·재방문 목적의 로그인을 요구하지 않고, 첫 결과 확인 후 저장 또는 재방문을 시도할 때 로그인 요청을 표시한다.

`[TBD — 공유 객체 비로그인 열람 가능 여부와 이에 따른 실제 로그인 시점이 확정된 뒤 나머지 AC를 작성한다.]`
...
- **AC-09-01:** B가 참여하지 않았거나 아직 조건을 입력하지 않은 상태에서도 A는 막히지 않고, 자신의 조건만 적용된 후보별 실부담·판정과 해당하는 경우 통근 결과를 볼 수 있다.
- **AC-09-02:** 1인 빈 경로에서는 계산과 판정이 A 1인 입력만 반영한 결과임을 명시하고, A가 `출근 안 함`인 경우 통근·교통비 행을 표시하지 않는다.
...
> **System Behavior / Output:** 조건별 충족·미충족·미달량·확인 필요 산출  
> **우선순위:** 단계 1  
> **상태:** `[확정]`

**Acceptance Criteria**

- **AC-10-01:** 사람의 조건이나 후보 매물이 추가·수정되면 시스템은 관련 후보에 예산·통근시간·역 도보·전용면적·주차·매물 유형 중 적용되는 조건을 다시 판정한다.
- **AC-10-02:** 판정 결과는 사람별·조건별 충족 또는 미충족과 실제 미달량을 표시하고, 자동 판정할 수 없는 항목은 `확인 필요`로 분리하며 하나의 공동 총점을 생성하지 않는다.
...
- **AC-11-01:** A와 B의 현재 입력이 반영되면 시스템은 각 후보를 `둘 다 충족`, `한쪽만 충족`, `둘 다 불충족` 중 하나의 그룹에 표시한다.
- **AC-11-02:** 확인이 필요한 후보에는 목록에서 최소 수준의 `확인 필요` 배지를 별도로 표시하고, 해당 상태를 충족·미충족 그룹을 대체하는 종합 판정이나 점수로 사용하지 않는다.
...
- **AC-12-01:** 사용자가 후보 상세에 진입하면 시스템은 `예산 → 통근 → 추가 필수①~④ → 확인 필요` 순서를 유지해 A/B의 조건별 실제값·기준값·미달량을 나란히 표시한다.
- **AC-12-02:** 한쪽만 충족하는 후보의 양보 문장은 양보 주체·조건·실제 미달량을 밝히고, 이점이 있으면 후보 5개 안에서의 조건별 상대적 이점만 표시한다. 이점이 없으면 `대신` 절을 만들지 않으며, 미충족 조건이 3개 이상이면 문장에는 고정 순서상 앞 2개만 넣고 나머지는 목록으로 표시한다.
- **AC-12-03:** 상세 설명에는 특정 후보가 더 좋거나 공정하다는 결론, 추천 배지, 공동 적합도 총점을 표시하지 않는다.
...
- **AC-13-01:** 사용자가 조건 완화 화면에 진입하면 시스템은 A안과 B안을 동시에 표시하며, 각 완화폭은 현재 후보의 실제 미달량을 기준으로 한다.
- **AC-13-02:** 사용자가 슬라이더를 조작하면 조건 변경을 확정하기 전에 현재 후보의 판정 등급이 어떻게 바뀌는지 즉시 미리보기로 표시한다.
- **AC-13-03:** 시스템은 두 조건을 동시에 완화하는 안이나 실제 미달량과 무관한 임의 완화폭을 제시하지 않는다.
...
- **AC-14-01:** 사용자가 상대의 조건을 완화하려 하면 시스템은 해당 조건을 직접 변경하지 않고 상대에게 변경 제안을 발송한다.
- **AC-14-02:** 제안은 상대가 수락하기 전까지 판정에 적용되지 않으며, 상대가 수락한 경우에만 해당 사람의 조건을 갱신하고 후보를 재판정한다.
...
- **AC-15-01:** `둘 다 충족`과 `한쪽만 충족` 후보가 모두 0개이면 시스템은 한 번에 한 조건만 완화하는 시뮬레이션을 실행하고, 후보가 살아나는 경우 해당 조건의 완화 경로를 표시한다.
- **AC-15-02:** 어떤 조건 하나를 완화해도 후보가 살아나지 않으면 시스템은 조건 완화 대신 재탐색 필터 제안 경로를 표시하며, 두 조건을 동시에 완화하는 안은 제시하지 않는다.
...
- **AC-16-01:** 시스템은 제안 필터를 자동 적용하지 않고, 사용자가 `이 필터로 찾아보기`를 선택했을 때만 네이버 검색으로 이동시킨다.
- **AC-16-02:** 전달 가능한 조건은 예산의 더 낮은 상한, 전용면적의 더 높은 하한, 더 짧은 역 도보 기준, 한 명이라도 필요로 하는 주차 조건을 사용한다. 매물 유형은 하나의 필터로 표현 가능한 경우에만 전달하고 통근시간은 전달하지 않는다.
...
- **AC-17-01:** 선택 가능한 후보가 2개 이상일 때 A와 B는 각각 후보 2개를 선택할 수 있고, 두 선택이 모두 일치하면 시스템은 그 2개를 방문 후보로 확정한다.
- **AC-17-02:** 일치 후보가 1개이면 시스템은 일치한 후보를 유지하고 남은 한 자리에 대한 후보별 조건 차이를 보여준 뒤 한 번 더 선택하게 한다.
- **AC-17-03:** 일치 후보가 0개이거나 최대 2라운드 후에도 남은 자리가 일치하지 않으면 시스템은 각자가 선택한 한 곳씩을 방문 후보로 구성하며, 투표·순위·자동 선택으로 대체하지 않는다.
...
- **AC-18-01:** 새 매물을 추가하거나 기존 매물을 교체해도 A/B의 예산·통근·추가 필수·선호·확인 항목은 사람에게 유지되고 다시 입력하도록 요구하지 않는다.
- **AC-18-02:** 시스템은 유지된 사람 조건을 새 매물에 자동 적용해 조건별 판정·미달량·확인 필요 상태를 산출한다.
...
- **AC-19-01:** 후보의 거래완료 또는 삭제가 감지되면 시스템은 해당 매물을 현재 후보에서 제거하고 두 사용자에게 상태 변화를 알리되 사람 조건과 나머지 후보의 판정은 유지한다.
- **AC-19-02:** 소진된 매물이 확정된 방문 후보였다면 시스템은 남아 있는 방문 후보를 유지하고 비어 있는 한 자리 선택 단계로 되돌린다.
...
- **AC-20-01:** 실제값을 계산했고 사용자 기준에 못 미친 경우에만 `미충족`으로 표시하고 실제 미달량을 함께 보여준다.
- **AC-20-02:** 경로 또는 데이터 계산을 시도했지만 실패한 경우 `계산 불가`로 표시하고 `미충족`으로 분류하지 않으며, 데이터가 없어 사람이 확인해야 하는 항목은 `확인 필요`로 분리한다.
- **AC-20-03:** 사용자가 `출근 안 함`을 선택한 경우 통근은 `해당 없음`으로 처리해 비교표의 통근 행 자체를 표시하지 않고 `계산 불가`로 표시하지 않는다.
...
- **AC-21-01:** 시스템이 실부담·교통비·금리 기반 수치를 표시할 때 각 수치와 함께 계산 기준 시점, 사용한 가정과 적용 한계를 확인할 수 있어야 한다.
- **AC-21-02:** 계산에 필요한 값이나 기준이 없어 확정적으로 산출할 수 없는 경우 전제 없는 숫자를 표시하지 않고 `계산 불가` 또는 `확인 필요`로 구분한다.
...
- **AC-22-01:** 단계 2 진입 시 `확인 필요` 항목이 있으면 시스템은 해당 항목을 방문 전 중개사 질문 목록으로 표시한다.
- **AC-22-02:** 사용자가 중개사 답변을 기록하면 시스템은 그 답변을 해당 매물과 질문에 연결해 저장하고 관련 보류 상태를 갱신한다. 답변이 없는 항목은 `확인 필요` 상태로 유지한다.
- **AC-22-03:** 중개사 질문 카드에는 방문 후 공통 체크리스트를 섞어 표시하지 않는다.
...
- **AC-23-01:** 방문 완료 후 시스템은 층간소음·실제 채광·곰팡이·주차 난이도·주변 소음·내부 상태의 공통 체크리스트를 모두 표시하며, 사용자가 체크리스트 항목 자체를 선택·삭제하도록 요구하지 않는다.
- **AC-23-02:** 사용자가 체크리스트와 `유지·보류·제외` 상태를 입력하면 시스템은 이를 해당 매물의 방문 후 사용자 생성 기록으로 저장하고 방문 전 `확인 필요` 기록과 분리한다.
...
- **AC-24-01:** 한 사용자의 조건 입력·변경 또는 조건 완화 제안이 발생하면 시스템은 다른 사용자에게 해당 상태 변화를 알린다.
- **AC-24-02:** 후보가 추가·교체·소진되거나 방문 후보 선택 상태가 바뀌면 시스템은 상대에게 어떤 후보 또는 선택 상태가 변경되었는지 알린다.
...
- **AC-25-01:** 판정·trade-off·최종 비교 화면은 A와 B의 조건을 동일한 조건 순서와 시각적 비중으로 표시하며 한쪽 정보만 우선 배치하지 않는다.
- **AC-25-02:** 두 사람의 조건을 하나의 공동 적합도·총점·일치율로 합산하지 않고, 추천 배지·복합 순위·AI의 최종 선택을 표시하지 않는다.
- **AC-25-03:** A/B는 위치와 라벨로 구분하고 색은 충족·미충족·확인 상태에 사용한다. 겹치는 지도 경로선은 최신 결정의 예외에 따라 A/B 구분색을 사용할 수 있다.
...
| FR-06 | 1 | 확정 | 조건 입력 전 후보·A 선호 맥락 표시 |
| FR-07 | 3 | 일부 TBD | 임시 저장 정책 확정, 비로그인 진입 가능 여부는 네이버 정책 TBD |
| FR-08 | 1 | 일부 TBD | 결과 후 로그인 방향만 확정 |
| FR-09 | 2 | 확정 | B 미참여 시 A 1인 빈 경로 |
| FR-10 | 2 | 확정 | 최신 여섯 판정 항목·총점 금지 |
...
**합계:** 작성 완료 AC 56개 · 확정 20개 FR · 일부 TBD 5개 FR · 전체 TBD 0개 FR
