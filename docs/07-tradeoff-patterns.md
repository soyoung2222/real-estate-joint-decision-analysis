...
| **개인 가치 우선 순서** — 중요하게 여기는 속성을 먼저 제시하면 가치일치 선택이 **70% → 90%** | 조건 표시를 **필수 → 선호 → 확인** 순서로 고정 | ⚠️ **명시 필요** |
...
| # | 조치 | 어디에 |
|---|---|---|
| 1 | **조건 표시 순서를 필수 → 선호 → 확인으로 고정** | 09·14번 화면 · G 신설 |
| 2 | **A/B 블록의 시각 비중을 동일하게** | 09·14번 화면 |
| 3 | **양보 문장의 손실·이득 순서를 양쪽 동일하게** | F5 명세 |
